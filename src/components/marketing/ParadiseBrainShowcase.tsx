...
export function ParadiseBrainShowcase({ traces = showcaseTraces }: { traces?: readonly ShowcaseTrace[] }) {
    ...
    <Section id="como-piensa" className="py-16 sm:py-20 md:py-24">
        <div className="max-w-3xl">
          ...
          <p className="p-text-muted mt-4 text-sm leading-relaxed md:text-base">
            Una demo curada que muestra cómo Paradise interpreta un pedido, detecta señales, recomienda módulos y genera
            una respuesta guiada. Sin debug crudo: producto explicable.
          </p>
        </div>
        ...
        <div className="mt-7 flex flex-wrap gap-2 sm:mt-8">
          {traces.map((t) => (
            <Button
              key={t.key}
              variant={t.key === active ? 'primary' : 'outline'}
              size="sm"
              onClick={() => setActive(t.key)}
              className={cn(t.key === active && 'shadow-[0_0_24px_-14px_var(--p-glow)]')}
            >
              {t.label}
            </Button>
          ))}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="rounded-full border border-[rgb(var(--p-accent-rgb)/0.4)] bg-[rgb(var(--p-accent-rgb)/0.12)] px-4 py-2 text-sm font-semibold text-white">
                      {trace.routing.primary}
                    </span>
                    {trace.routing.secondary && (
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  ...
                  <div className="mt-3 grid gap-3 text-sm text-slate-300 sm:grid-cols-2">
                    {trace.context.goals?.length ? (
                      <div>
                        <span className="text-xs uppercase tracking-wider text-slate-500">objetivos</span>
                        <ul className="mt-1 space-y-1">
                          {trace.context.goals.map((g) => (
                            <li key={g}>• {g}</li>
                          ))}
                        </ul>
                      </div>
                    ) : null}
                    {trace.context.priorities?.length ? (
                      <div>
                        <span className="text-xs uppercase tracking-wider text-slate-500">prioridades</span>
                        <ul className="mt-1 space-y-1">
                          {trace.context.priorities.map((p) => (
                            <li key={p}>• {p}</li>
                          ))}
                        </ul>
                      </div>
                    ) : null}
                    {trace.context.workflows?.length ? (
                      <div className="sm:col-span-2">
                        <span className="text-xs uppercase tracking-wider text-slate-500">flujos</span>
                        <ul className="mt-1 space-y-1">
                          {trace.context.workflows.map((w) => (
                            <li key={w}>• {w}</li>
                          ))}
                        </ul>
                      </div>
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  ...
                  <div className="mt-4 space-y-3">
                    <div>
                      <span className="text-xs uppercase tracking-wider text-slate-500">reglas</span>
                      <ul className="mt-1 space-y-1 text-sm text-slate-300">
                        {trace.knowledge.rules.map((r) => (
                          <li key={r}>• {r}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <span className="text-xs uppercase tracking-wider text-slate-500">métricas</span>
                      <p className="mt-1 text-sm text-slate-400">{trace.knowledge.metrics.join(', ')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  ...
                  <div className="artifact-card mt-3 rounded-[16px] border border-emerald-400/25 bg-[linear-gradient(180deg,rgba(16,185,129,0.06),rgba(255,255,255,0.02))] p-4 sm:rounded-[20px] sm:p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {trace.response.nextSteps.map((s) => (
                        <span
                          key={s}
                    ...
                    <p className="p-text-muted mt-4 text-sm italic">{trace.response.followUp}</p>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
