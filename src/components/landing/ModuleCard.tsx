...
export function ModuleCard({ module, highlighted, highlightTags, compact }: ModuleCardProps) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        ...
        compact ? 'p-5' : 'p-6',
        computedHighlighted &&
          'border-[rgb(var(--p-accent-rgb)/0.55)] shadow-[0_0_32px_rgba(124,58,237,0.18)]',
      )}
    >
      ...
      <div className="mt-4">
        ...
        {highlights.length ? (
          <ul className={cn('p-text-muted mt-4 grid gap-2 text-xs', compact ? 'max-w-none' : 'max-w-[50ch]')}>
            {highlights.map((h) => (
        ...
        {!compact && specTags.length ? (
          <div className="mt-5 flex flex-wrap gap-2">
            {specTags.slice(0, 5).map((tag) => {
              const emphasized = highlightTags?.includes(tag)
              return (
                <Pill
                  key={tag}
                  className={cn(
                    emphasized && 'p-chip-active',
                  )}
                >
                  {tag}
                </Pill>
      ...
      <footer className={cn('mt-6 flex flex-wrap gap-3', compact && 'mt-5')}>
        <Link
          href={module.repoUrl}
          muted
          className="p-btn-secondary inline-flex items-center gap-2 rounded-[var(--radius-pill)] px-4 py-2 text-xs font-semibold"
        >
